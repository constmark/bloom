...
mod tests {
    ...
    fn test_security_features_sequentially() {
        ...
        std::env::set_var("BLOOM_STRICT_SECURITY", "true");
        assert!(is_strict_security());
        std::env::set_var("BLOOM_STRICT_SECURITY", "0");
        assert!(!is_strict_security());
        ...
        std::env::set_var("BLOOM_ALLOWED_SCRIPTS", "evil.py,test.py");
        assert!(validate_external_script(Path::new("evil.py")).is_ok());
        // Normal mode warns but allows
        std::env::set_var("BLOOM_STRICT_SECURITY", "0");
        std::env::remove_var("BLOOM_ALLOWED_SCRIPTS");
        assert!(validate_external_script(Path::new("evil.py")).is_ok());
        ...
        std::env::set_var("BLOOM_ALLOWED_RUNNERS", "evil-runner");
        ...
        std::env::set_var("BLOOM_ALLOWED_PLUGINS", "com.evil.plugin");
        assert!(validate_plugin("com.evil.plugin").is_ok());
        // Test mock/test plugins are allowed
        assert!(validate_plugin("test-plugin").is_ok());

        // Cleanup
        std::env::remove_var("BLOOM_STRICT_SECURITY");
        std::env::remove_var("BLOOM_ALLOWED_SCRIPTS");
        std::env::remove_var("BLOOM_ALLOWED_RUNNERS");
        std::env::remove_var("BLOOM_ALLOWED_PLUGINS");
    }
}
